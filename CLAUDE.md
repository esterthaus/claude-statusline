...
1. **Model + Context + Usage**: Model name, context window progress bar, then flavor-specific trailing elements — Claude Code: 5h/7d rate limits with reset times (`rate_limits`) plus one element per model-specific weekly limit (e.g. `Fable`, fetched from the OAuth usage endpoint, see below); Copilot: AIU consumption (`ai_used.formatted`) + session lines added/removed. Each Claude Code rate limit also carries a projection (`→74%`, `projectUsage()`): the window's average burn so far extrapolated to the window end — window start is `resets_at` minus the window length (5 h / 7 d), suppressed below `minElapsedFraction` of the window elapsed, capped at 999 %, own traffic lights via `getColorForProjection()` (<90 green, ≤110 amber, >110 red). Reset times beyond 24 h render as `2d 5h` / `Sa 14:44` (`formatDuration()`, `formatResetClock()`). Elements are dropped from the right as the terminal narrows (`termWidth/40` elements max: <80 → 1, <120 → 2, ≥120 → 3)
